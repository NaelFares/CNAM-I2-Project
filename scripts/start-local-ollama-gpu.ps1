...
$composeArgs = @("-f", "docker/docker-compose.ollama.yml", "-f", "docker/docker-compose.gpu.yml")
$upArgs = @($composeArgs)
...
Write-Host "Mode IA : Ollama local GPU" -ForegroundColor Cyan
Write-Host "Ollama  : actif, image et modele telecharges au premier lancement" -ForegroundColor Yellow

docker compose @upArgs
if ($LASTEXITCODE -ne 0) {
  Write-Host "Le demarrage GPU a echoue. Utilise start-local-ollama.ps1 pour le mode CPU." -ForegroundColor Yellow
  exit $LASTEXITCODE
}
